...
export class ViewerBridge {
  ...
  private selectionWritesEnabled = true;
  ...
  publishSelection(selection: ViewerSelection, isCurrent: () => boolean = () => true) {
    const payload = createSelectionPayload(selection);
    return this.writeSelection(() => api.setSelection(payload), isCurrent);
  }

  async closeActiveModel(activation: ActivateModelResponse) { await selectionQueue; return api.cancelModelLoad(activation); }

  clearSelection(isCurrent: () => boolean = () => true) {
    return this.writeSelection(() => api.clearSelection(), isCurrent);
  }
  private writeSelection(write: () => Promise<unknown>, isCurrent: () => boolean) {
    const task = selectionQueue.then(async () => {
      if (!isCurrent() || !this.selectionWritesEnabled) return;
      try { await write(); }
      catch (error) { if (isCurrent()) console.warn(`Selection bridge: ${this.errorText(error)}`); }
    });
  ...
  async watchModel(
    ...
    this.stopWatching();
    const preparation = { controller: new AbortController(), activated };
    ...
    try {
      ...
      const readRuntime = async (watchSignal: AbortSignal) => {
        ...
        if (!runtime.hasActiveModel || runtime.activeModelHash !== modelHash || (runtime.activeLoadedAt && runtime.activeLoadedAt !== activated.loadedAt)) throw new Error("Backend active model changed during INDEX preparation");
